...
int main(int, char**) {
	...
	// setting windows
	std::map<std::string, bool> window_triggers{
		{"show_cast_window", true},
		{"page_setting", true},
		{"casts_list", true},
	...
	// bool show_element_detail = false;
	...
	Cast cast(game_data_ptr,
			  std::make_unique<Backup>(backup_game_data),
			  std::make_unique<Page>(clipboard_page),
			  std::make_unique<std::map<std::string, bool>>(window_triggers));
